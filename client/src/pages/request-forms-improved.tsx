...
export default function RequestForms() {
  ...
  const userOvertimeRequests = (overtimeRequests as any[]).filter(
            ...
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">
                  {((userLeaveRequests as any[]).filter(req => req.status === 'pending').length + 
                    userOvertimeRequests.filter(req => req.status === 'pending').length)}
                </p>
        ...
        <CardContent>
          {userRequestsLoading || overtimeLoading ? (
            ...
            <div className="space-y-4">
              {/* Leave Requests */}
              {(userLeaveRequests as any[]).slice(0, 3).map((request: any) => (
              ...
              {(userLeaveRequests.length === 0 && userOvertimeRequests.length === 0) && (
                <div className="text-center py-8">
